...
Page({
  ...
  getAIResponse(userMessage) {
    // 简单的关键词匹配回复（实际应用应该调用后端API）
    let aiContent = '';
    const lowerMessage = userMessage.toLowerCase();

    if (lowerMessage.includes('要求') || lowerMessage.includes('需要做什么') || lowerMessage.includes('任务内容')) {
      aiContent = `根据任务"${this.data.taskName}"的要求：${this.data.taskDesc}。你需要深入理解任务目标，制定详细的执行计划，并按照步骤逐步完成。建议先从任务分析开始，明确每个环节的具体要求和验收标准。`;
    } else if (lowerMessage.includes('步骤') || lowerMessage.includes('如何') || lowerMessage.includes('怎么')) {
      aiContent = `完成任务"${this.data.taskName}"的一般步骤包括：\n1. 明确任务目标和具体要求\n2. 收集和整理相关资料\n3. 制定详细的执行计划\n4. 按计划逐步实施\n5. 进行测试和验证\n6. 完善和优化\n7. 总结和反思\n\n你可以根据具体情况调整这些步骤。`;
    } else if (lowerMessage.includes('困难') || lowerMessage.includes('问题') || lowerMessage.includes('遇到')) {
      aiContent = `遇到困难是很正常的，不要担心！建议你：\n1. 先明确具体遇到了什么问题\n2. 查阅相关文档和资料\n3. 尝试多种解决方案\n4. 如果还是无法解决，可以向老师或同学寻求帮助\n5. 记住每次解决问题的经验\n\n能详细说说你遇到的具体问题吗？我可以帮你分析。`;
    } else if (lowerMessage.includes('工具') || lowerMessage.includes('软件') || lowerMessage.includes('技术')) {
      aiContent = `针对"${this.data.taskName}"这个任务，建议使用以下工具和技术：\n• 根据任务性质选择合适的开发工具\n• 使用版本控制工具管理代码（如Git）\n• 参考官方文档和最佳实践\n• 合理利用AI辅助工具提高效率\n\n你想了解哪个具体工具的使用方法吗？`;
    } else {
      // 默认回复
      ...
      aiContent = defaultReplies[Math.floor(Math.random() * defaultReplies.length)];
    }
